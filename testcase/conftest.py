...
# 全局 fixture：每个测试用例 开始/结束 都自动打印日志
...
# 会话级 fixture：整个测试【只执行一次】，所有用例执行前自动登录
...
def datadb_init():
    ...
    pass
#  pytest 全局前后置配置：自动打印用例开始 / 结束日志、测试前自动登录、预留数据库清理功能。

# 存放用例层专属的 fixture / 前后置，例如：
# 1. 你截图里的用例日志打印、全局登录、数据库清理
# 2. 接口测试专属的 token 提取、用例前后置
# 3. 业务层专属的 fixture（如订单 / 商品模块专属）